...
Copy the scripts to the server.
	...
	scp -r -i ~/.ssh/server001keypair.pem src ec2-user@ec2-184-73-84-106.compute-1.amazonaws.com:/tmp

Login to the server and run the scripts.

	ssh -i ~/.ssh/server001keypair.pem ec2-user@ec2-184-73-84-106.compute-1.amazonaws.com
	cd   /tmp/src/scripts
	bash new_gemset.sh
	bash setup.sh
...
= Part 3: 000_cap_deploy.sh will be used by other examples to deploy applications
