...
Follow the instructions on https://rvm.beginrescueend.com/rvm/install/ for Multi-User installation as root (for server administrators).
	...
	cd 001_rvm
	scp -r -i ~/.ssh/server001keypair.pem server ec2-user@ec2-50-17-56-198.compute-1.amazonaws.com:/tmp
	ssh -i ~/.ssh/server001keypair.pem ec2-user@ec2-50-17-56-198.compute-1.amazonaws.com
	cd   /tmp/server
	sudo bash 000_install_rvm.sh
